...
import { TRIGRAMS, STEMS, BRANCHES, getSixBeasts, getSixQin } from './config.js';

const POSITION_NAMES = ['初爻','二爻','三爻','四爻','五爻','上爻'];

const LINE_OLD_YIN    = 0;
const LINE_YOUNG_YIN  = 1;
const LINE_YOUNG_YANG = 2;
const LINE_OLD_YANG   = 3;
...
export function castHexagram() {
  ...
  // 4. 获取日月建
  const { dayStem, dayBranch, monthStem, monthBranch, yearStem, yearBranch } = getCurrentDayStemBranch();
  ...
  const displayLines = originalHexagram.lines.map((lineData, idx) => {
    ...
    return {
      ...
      changedSixQin: changedLineData
        ? getSixQin(originalHexagram.palaceElement, changedLineData.element)
        : null,
