...
function formatInterpretation(text) {
  // 先转义 HTML，防止 AI 输出中的恶意标签被执行
  let safe = escapeHtml(text);
  // 在安全文本上应用格式化（标记字符不是 HTML，不会被转义）
  safe = safe.replace(/【原文[：:]\s*["“](.+?)["”]\s*】/g, '<span class="quote">📖 $1</span>');
  safe = safe.replace(/【原文[：:]\s*(.+?)】/g, '<span class="quote">📖 $1</span>');
